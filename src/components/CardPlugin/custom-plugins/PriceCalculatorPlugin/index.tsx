  ...
  const plugin: CardPlugin = {
    ...
    hooks: {
      renderFooter: (context) => {
        const skuPrice = context.bus?.getData?.("sku.price");
        const price = Number(
          (skuPrice as any) ?? (context.data as any)?.price ?? 0
        );
        const originalPrice = Number((context.data as any)?.originalPrice || 0);
        const hasOriginal = showOriginalPrice && originalPrice > price && originalPrice > 0;
        const savings = hasOriginal ? originalPrice - price : 0;
        const discount = hasOriginal ? Math.round((1 - price / originalPrice) * 100) : 0;

        const quantity = Number(context.bus?.getData?.("quantity") ?? 1);
        const appliedCoupon = context.bus?.getData?.("coupon.applied") as { code?: string; discount?: number } | undefined;
        ...
        return (
          <div style={{ padding: 12 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              ...
              {hasOriginal && showDiscount && (
                <span style={{ fontSize: 12, color: "#388e3c", fontWeight: 700 }}>-{discount}%</span>
              )}
            </div>

            {(hasOriginal && showSavings) && (
              <div style={{ marginTop: 4, fontSize: 12, color: "#555" }}>
                已省 {formatCurrency(savings, currency, locale)}
              </div>
            )}
            ...
            {showTotalPrice && (
              <div style={{ marginTop: 6, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: "#1e88e5" }}>
                  合计 {formatCurrency(total, currency, locale)}
                </span>
              </div>
            )}
          </div>
